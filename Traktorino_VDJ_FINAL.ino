/*
  Traktorino firmware, VirtualDJ final version

  Changes from original:
  1. MIDI_CREATE_DEFAULT_INSTANCE() enabled.
  2. MIDI.begin(MIDI_CHANNEL_OMNI) added for reliable MIDI input feedback.
  3. Button LED Note On velocity 0 is treated as Note Off.
  4. VU handling rewritten for VirtualDJ:
     - separate left/right VU tracking
     - accepts either 0-7 or 0-127 input values
     - clears VU LEDs before redrawing
     - clears stale VU after timeout when VirtualDJ stops sending level values
  5. MIDI.read() is drained each loop for more responsive LED/VU feedback.

  Shift-layer behaviour is handled in VirtualDJ mapping, not firmware.
*/
...
const unsigned long vuTimeoutMs = 220; // Tune 150-300ms if needed
...
void setVuMeter(unsigned int vuPins[7], byte level) {
  ...
  // Accept both compact 0-7 values and normal MIDI 0-127 values.
  if (level > 7) {
  ...
  // Clear all VU LEDs first.
  for (int i = 0; i < 7; i++) {
    ShiftPWM.SetOne(vuPins[i], LOW);
  }

  // First five LEDs are green.
  for (int i = 0; i < level && i < 5; i++) {
    ShiftPWM.SetOne(vuPins[i], green);
  }

  // Sixth LED is yellow.
  if (level >= 6) {
    ShiftPWM.SetOne(vuPins[5], yellow);
  }

  // Seventh LED is red.
  if (level >= 7) {
    ShiftPWM.SetOne(vuPins[6], red);
  }
}
...
void handleControlChange(byte channel, byte number, byte value) {
  ...
  // Convert 0-127 MIDI values to the firmware's 0-7 VU range.
  if (level > 7) {
    level = map(level, 0, 127, 0, 7);
  }
...
void handleNoteOn(byte channel, byte number, byte value)
{
  ...
  // Some DJ software uses this convention for LED off messages.
  if (value == 0) {
